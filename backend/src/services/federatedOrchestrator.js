...
/**
 * federatedOrchestrator.js - UPDATED with automatic .pkl export
 * 
 * Add this after the job completion logic (around line 280-320)
 */

// After job completes successfully, add this:

async function exportModelToPkl(jobId, accuracy, participants) {
  /**
   * Automatically export completed model to .pkl format
   * Called when activeJob.status = 'completed'
   */
  try {
    ...
    const response = await axios.post('http://localhost:5000/api/federated/export-pkl', exportPayload);
    
    console.log('[Orchestrator] Model exported to PKL:', {
      jobId,
    ...
    console.error('[Orchestrator] PKL export error:', error.message);
    // Non-critical error - don't fail the job
  }
}

// In the job completion section (after all rounds):
if (activeJob.currentRound >= activeJob.totalRounds) {
  activeJob.status = 'completed';
  activeJob.endTime = new Date();
  
  // Calculate final accuracy (average of last round metrics)
  const finalAccuracy = activeJob.roundMetrics?.[activeJob.totalRounds]?.avgAccuracy || 0;
  
  console.log(`[Orchestrator] Job ${jobId} COMPLETED`);
  console.log(`  Rounds: ${activeJob.totalRounds}`);
  console.log(`  Final Accuracy: ${(finalAccuracy * 100).toFixed(2)}%`);
  console.log(`  Participants: ${activeJob.participants.join(', ')}`);
  
  // ✓ NEW: Export model to PKL automatically
  await exportModelToPkl(
    jobId,
    finalAccuracy,
    activeJob.participants
  );
  ...
  io.emit('job:completed', {
    jobId,
    status: 'completed',
    globalWeights: activeJob.globalWeights,
    accuracy: finalAccuracy,
    totalRounds: activeJob.totalRounds,
    downloadUrl: `/api/federated/download-model/model-job-${jobId}.pkl`
  });
}
