 ...
 *
 *  4. NEW /schema endpoint — clients call this BEFORE loading their CSV so
 *     every participant builds the same model architecture (same output
 *     layer size) regardless of which local labels they happen to have.
 *     This is what prevents round-2 silent failures where mohammad's
 *     [Benign, FTP-BruteForce, SSH-Bruteforce] and ammar's [Benign,
 *     DoS-Hulk, DoS-SlowHTTPTest] produced models with different output
 *     layouts that couldn't be aggregated coherently.
 */
...
// Public to any authenticated participant. Returns the global ordered list
// of class labels every client must use for their output layer.
router.get('/schema', authenticate, (_req, res) => {
  res.setHeader('Cache-Control', 'no-store');
  return res.status(200).json({ schema: fedOrch.getGlobalSchema() });
});
