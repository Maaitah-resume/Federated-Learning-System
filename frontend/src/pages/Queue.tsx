...
import {
  ...
  Upload, FileText, X, Zap, Brain, Send, Lock, AlertTriangle,
} from 'lucide-react';
...
export default function Queue() {
  ...
  const [status, setStatus] = useState<TrainingStatus>({ phase:'idle',round:0,epoch:0,totalEpochs:3,accuracy:null,loss:null,message:'' });
  ...
  const currentId: string|null = (() => { try { const s=localStorage.getItem('fl_participant'); return s?JSON.parse(s):null; } catch { return null; } })();
  ...
  const runLocalRound = useCallback(async (event:{jobId:string;round:number;totalRounds:number;globalWeights:any|null}) => {
    ...
    try {
      ...
      setStatus({phase:'training',round:event.round,epoch:0,totalEpochs:EPOCHS,accuracy:null,loss:null,message:`Round ${event.round}/${event.totalRounds} — training locally…`});
      const metrics = await localTrainer.trainRound(EPOCHS, 32, (epoch,logs) => {
        setStatus(s=>({...s,epoch:epoch+1,accuracy:logs?.acc??null,loss:logs?.loss??null,message:`Epoch ${epoch+1}/${EPOCHS} — acc: ${((logs?.acc||0)*100).toFixed(1)}%`}));
      });
      ...
      setStatus(s=>({...s,phase:'waiting',accuracy:metrics.accuracy,loss:metrics.loss,message:`Round ${event.round} submitted — waiting for other nodes…`}));
    } catch (err:any) {
      console.error('[Queue] Round error:', err);
      setStatus(s=>({...s,phase:'idle',message:`Error: ${err.message}`}));
    } finally { isTrainingRef.current = false; }
  }, []);
  ...
  const handleFileSelect = useCallback(async (selected:File) => {
    ...
    try {
      ...
      if (pendingRoundRef.current && inQueueRef.current) {
        console.log('[Queue] CSV ready — starting pending round');
        const pending = pendingRoundRef.current;
        pendingRoundRef.current = null;
        runLocalRound(pending);
      }
    } catch (err:any) {
  ...
  useEffect(() => {
    if (!socket) return;
    const onRoundStarted = (data:any) => { if (inQueueRef.current) runLocalRound(data); };
    const onWeightsSub   = (data:SubmissionTracker) => setSubmissions(data);
    const onComplete     = () => { setStatus({phase:'done',round:0,epoch:0,totalEpochs:3,accuracy:null,loss:null,message:'Training complete!'}); pendingRoundRef.current=null; localTrainer.dispose(); refresh(); };
    socket.on('round:started',     onRoundStarted);
    socket.on('weights:submitted', onWeightsSub);
    socket.on('training:complete', onComplete);
    return () => { socket.off('round:started',onRoundStarted); socket.off('weights:submitted',onWeightsSub); socket.off('training:complete',onComplete); };
  ...
  const leaveQueue = async () => { setLeaving(true); try { await apiClient.post('/api/queue/leave'); await refresh(); } catch(e){console.error(e);} finally{setLeaving(false);} };
  const clearFile  = (e:React.MouseEvent) => { e.stopPropagation(); setFile(null); setDataReady(false); setDataInfo(null); setParseError(null); localTrainer.dispose(); pendingRoundRef.current=null; };
  ...
  const phaseStyle: Record<TrainingPhase,string> = {
    ...
    masking:'bg-violet-50 text-violet-700', submitting:'bg-blue-50 text-blue-600', done:'bg-emerald-100 text-emerald-800',
  };
      ...
      <AnimatePresence>
        {isJobRunning && (
          <motion.div initial={{opacity:0,y:-20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}}
            ...
            {submissions && <div className="mt-3 pt-3 border-t border-white/20 text-xs text-white/80 flex items-center gap-2"><Send size={12}/> Masked submissions: <span className="font-bold">{submissions.received}/{submissions.expected}</span> nodes</div>}
          </motion.div>
        )}
      </AnimatePresence>
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className={`bg-white p-6 rounded-[2rem] shadow-sm border-2 transition-all ${hasPendingRound?'border-red-400':'border-slate-100'}`}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="font-bold text-slate-900">Load Private Data {hasPendingRound&&<span className="ml-1 text-red-500 text-xs font-bold animate-pulse">← NOW!</span>}</h3>
            </div>
            <div onDrop={handleDrop} onDragOver={(e)=>{e.preventDefault();setDragOver(true);}} onDragLeave={()=>setDragOver(false)}
              className={`border-2 border-dashed rounded-2xl p-6 text-center cursor-pointer transition-colors ${hasPendingRound?'border-red-400 bg-red-50':dragOver?'border-indigo-400 bg-indigo-50':'border-slate-200 hover:border-indigo-300'}`}
              ...
              {status.phase==='loading'&&!file ? (
                <div className="flex flex-col items-center gap-2 text-indigo-600"><Loader2 className="animate-spin" size={28}/><p className="text-sm font-medium">Parsing locally…</p></div>
              ) : file ? (
                ...
                <p className={`text-sm font-medium ${hasPendingRound?'text-red-600':'text-slate-500'}`}>{hasPendingRound?'⚡ Drop CSV to start NOW!':'Drop your CSV here'}</p>
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 text-center relative overflow-hidden">
            ...
            {isJobRunning ? (
                ...
                {inQueue&&status.phase!=='idle'&&(
                  <div className={`rounded-2xl px-4 py-3 text-sm font-medium ${phaseStyle[status.phase]}`}>
                    ...
                    {status.phase==='training'&&(
                      <div className="mt-2">
                        <div className="flex justify-between text-xs opacity-70 mb-1"><span>Epoch {status.epoch}/{status.totalEpochs}</span>{status.accuracy!=null&&<span>acc {(status.accuracy*100).toFixed(1)}%</span>}</div>
                        <div className="h-1.5 bg-black/10 rounded-full overflow-hidden">
                          <motion.div className="h-full bg-current rounded-full opacity-60" animate={{width:`${(status.epoch/status.totalEpochs)*100}%`}} transition={{duration:0.3}}/>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <>
                ...
                {inQueue&&slotsLeft>0&&<div className="mb-4 p-2 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-700 font-medium">Need {slotsLeft} more {slotsLeft===1?'participant':'participants'} to start</div>}
                <AnimatePresence mode="wait">
                  {!inQueue ? (
                    <motion.button key="join" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}}
                      ...
                      className={`w-full py-4 rounded-2xl font-bold text-lg transition-all ${dataReady?'bg-indigo-600 text-white shadow-xl shadow-indigo-100 hover:bg-indigo-700 cursor-pointer':'bg-slate-200 text-slate-400 cursor-not-allowed'}`}>
                      {joining?<span className="flex items-center justify-center gap-2"><Loader2 className="animate-spin" size={20}/> Joining...</span>:dataReady?'Join Waiting Room':'🔒 Load CSV First'}
                    </motion.button>
                  ) : (
                    <motion.div key="waiting" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}} className="space-y-3">
                      ...
                      <button onClick={leaveQueue} disabled={leaving} className="w-full border border-red-200 text-red-500 py-3 rounded-xl text-sm font-medium hover:bg-red-50 transition-all disabled:opacity-50">{leaving?'Leaving…':'Leave Queue'}</button>
                    </motion.div>
                  )}
                </AnimatePresence>
              </>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
            <div className="p-6 border-b border-slate-50 flex justify-between items-center">
              <div><h2 className="text-xl font-bold text-slate-900">{isJobRunning?'Training Nodes':'Connected Nodes'}</h2>
              {isJobRunning&&<p className="text-xs text-slate-400 mt-0.5">Round {activeJob.currentRound} of {activeJob.totalRounds} — {activeJob.status}</p>}</div>
            ...
            :queue.companies.length===0?(<div className="p-12 text-center text-slate-400"><Users size={40} className="mx-auto mb-3 opacity-30"/><p className="font-medium">No nodes connected yet</p></div>)
